...
def _is_challenge(title):
    import re
    global _CHALLENGE_RE
    if _CHALLENGE_RE is None:
        _CHALLENGE_RE = re.compile(r'just a moment|attention required|checking your browser|un momento|verifying', re.I)
    return bool(_CHALLENGE_RE.search(title or ""))

def fetch_html(url, wait_sel=None, headful=False, settle=2.5, timeout=60000):
    """Playwrightでurlを開き、Cloudflareチャレンジが自動で晴れるのを最大~28s待ってからHTMLを返す。
    返り値: (html, title)。title が 'Just a moment' 等ならチャレンジ未通過。"""
    ...
    with sync_playwright() as p:
        ...
        # webdriver フラグを隠す（簡易stealth）
        ctx.add_init_script("Object.defineProperty(navigator,'webdriver',{get:()=>undefined});")
        page = ctx.new_page()
        try:
            page.goto(url, wait_until="domcontentloaded", timeout=timeout)
            # チャレンジ(非対話型)が自動リダイレクトで晴れるのを待つ
            for _ in range(14):
                title = page.title() or ""
                if not _is_challenge(title): break
                time.sleep(2)
...
def sniff(key, cfg, url, headful):
    ...
    html, title = fetch_html(url, headful=headful)
    fn = f"{key}_pw.html"
    try: open(fn, "w", encoding="utf-8").write(html)
    except Exception: pass
    print(f"  {len(html):,}bytes / dump: {fn}")
    print(f"  title = {title[:70]!r}" + ("   ← ★チャレンジ未通過（headlessでは越えられない）" if _is_challenge(title) else "   ← 通過OK"))
...
def crawl(key, cfg, url, parser, pages, headful, page_param, page_tmpl):
    ...
    for pageno in range(1, pages + 1):
        ...
        html, title = fetch_html(purl, headful=headful)
        if _is_challenge(title):
            print(f"[{key}] p{pageno} チャレンジ未通過（title={title[:40]!r}）。中断。", file=sys.stderr); break
        if parser == "jsonld":
            items = []
