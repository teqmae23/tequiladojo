...
// ── Stripe Terminal（対面カード決済・サーバー主導） ───────────────
// スマート端末（WisePOS E / Stripe Reader S700 等）向け。端末ID(readerId)は
// settings/terminalConfig もしくはリクエストで指定する。
// JPYはゼロ十進通貨のため amount は「円」そのまま（×100しない）。
// 会計画面(checkin.html)から呼び出し、成功後に会計を確定する。
async function getTerminalReaderId(dataReaderId) {
  if (dataReaderId) return String(dataReaderId);
  const snap = await db.collection('settings').doc('terminalConfig').get();
  if (snap.exists && snap.data().readerId) return String(snap.data().readerId);
  throw new functions.https.HttpsError(
    'failed-precondition',
    'Stripe端末（reader）が未設定です。端末設定でreader IDを登録してください。');
}

// 決済を開始し、端末に金額を送信する。成功後は端末で顧客がカード操作を行う。
exports.terminalCollectPayment = functions
  .runWith({ secrets: ['STRIPE_SECRET_KEY'] })
  .https.onCall(async (data, context) => {
    await assertStaff(context);
    const stripe = require('stripe')(stripeSecretKey.value());
    ...
    const readerId = await getTerminalReaderId(data && data.readerId);
    const metadata = { source: 'checkout' };
    if (data && data.sessionDate) metadata.sessionDate = String(data.sessionDate);
    if (data && data.reference) metadata.reference = String(data.reference).slice(0, 400);
    let pi;
    try {
      ...
      throw new functions.https.HttpsError('internal', 'PaymentIntent作成に失敗: ' + e.message);
    }
    try {
      await stripe.terminal.readers.processPaymentIntent(readerId, { payment_intent: pi.id });
    } catch (e) {
      // 端末オフライン/使用中など。作成済みPIはキャンセルしておく。
      try { await stripe.paymentIntents.cancel(pi.id); } catch (_) { /* noop */ }
      throw new functions.https.HttpsError(
        'failed-precondition',
        '端末への送信に失敗しました（端末の電源・接続・使用中をご確認ください）: ' + e.message);
    }
    return { paymentIntentId: pi.id, status: pi.status, readerId };
  });

// PaymentIntentの状態を取得（クライアントがポーリングする）。
exports.terminalPaymentStatus = functions
  .runWith({ secrets: ['STRIPE_SECRET_KEY'] })
  .https.onCall(async (data, context) => {
    await assertStaff(context);
    const stripe = require('stripe')(stripeSecretKey.value());
    ...
    // 自動キャプチャ設定だが、万一 requires_capture で止まっていれば確定する。
    if (pi.status === 'requires_capture') {
      try {
        const cap = await stripe.paymentIntents.capture(pi.id);
        return { status: cap.status, amount: cap.amount };
      } catch (_) { /* fallthrough */ }
    }
    return {
      status: pi.status,
      amount: pi.amount,
      lastError: pi.last_payment_error ? pi.last_payment_error.message : null,
    };
  });

// 進行中の端末操作とPaymentIntentをキャンセルする。
exports.terminalCancelPayment = functions
  .runWith({ secrets: ['STRIPE_SECRET_KEY'] })
  .https.onCall(async (data, context) => {
    await assertStaff(context);
    const stripe = require('stripe')(stripeSecretKey.value());
    const piId = data && data.paymentIntentId;
    let readerId = data && data.readerId;
    if (!readerId) { try { readerId = await getTerminalReaderId(null); } catch (_) { readerId = null; } }
    if (readerId) { try { await stripe.terminal.readers.cancelAction(String(readerId)); } catch (_) { /* noop */ } }
    if (piId) { try { await stripe.paymentIntents.cancel(String(piId)); } catch (_) { /* noop */ } }
    return { ok: true };
  });

// 端末設定用: 登録済みのreader/locationを一覧する（設定画面で選択させる）。
exports.terminalListReaders = functions
  .runWith({ secrets: ['STRIPE_SECRET_KEY'] })
  .https.onCall(async (data, context) => {
    await assertStaff(context);
    const stripe = require('stripe')(stripeSecretKey.value());
    const params = { limit: 100 };
    if (data && data.locationId) params.location = String(data.locationId);
    const res = await stripe.terminal.readers.list(params);
    const readers = (res.data || []).map((r) => ({
      id: r.id, label: r.label, status: r.status, deviceType: r.device_type, location: r.location,
    }));
    let locations = [];
    try {
      const locs = await stripe.terminal.locations.list({ limit: 100 });
      locations = (locs.data || []).map((l) => ({ id: l.id, displayName: l.display_name }));
